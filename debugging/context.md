...
4. **This prompt is for planning and documentation design, not implementation.**
   - Produce or update the architecture docs first.
   - Then produce the debugging session directories under `debugging/`.
   - Do not assume you should mutate runtime code or fix bugs.

## Docs and local sources you must read first

Before producing the architecture docs or session directories, read and cite the
local project docs that materially affect the debugging plan:

- `HACKING.md`
- `AGENTS.md`
- `macos/AGENTS.md`
- `README.md`
- relevant docs under `tmux_docs/`
...
## tmux docs worth reading even if they are only on `tmux_mvp`

If the current branch does not have the full tmux design docs, it is acceptable
to read them directly from the `tmux_mvp` branch without checking out that
branch, for example with `git show`.

The most useful ones for this debugging plan are:

- `tmux_docs/context.md`
- `tmux_docs/overview.md`
- `tmux_docs/dataflow.md`
- `tmux_docs/surfaces_and_backends.md`
- `tmux_docs/subtask4.md`
- `tmux_docs/smallestmvp.md`

Use them as **comparison scaffolding**, not as the primary source of truth.
Current-branch code always wins if there is a conflict.

These docs are especially useful for:

- the distinction between `Surface` and `Backend`
- the read-thread versus app-thread handoff problem for tmux `.windows`
- the fact that `Viewer` and command round-tripping already exist while the
  GUI bridge is still missing on `main`
...
1. **Do not present unverified details as confirmed facts.**
   ...
   - If a line number matters, verify it against the current branch.
   - If a symbol location is uncertain, say so and explain how to confirm it.

2. **Separate three kinds of truth in your architecture docs.**
   - verified from code
...
Keep the canonical architecture write-up in:

- `debugging/architecture.md`

Keep the short summary in:

- `debugging/architecture_brief.md`

The architecture docs must confirm or correct the following on the current
branch and cite real file:line references verified there.
...
8. **What is still missing on `main`**
   - where `.windows` reaches `stream_handler.zig`
   - why that is not enough
   - why a future GUI bridge must cross to the app thread before creating
     surfaces

If any part of my framing is wrong, correct it in the architecture docs before
designing sessions.
...
- one subdirectory per approved session:
  ...
  - `s3_output_stack/`
  - `s4_resize/`
  - `s5_surface_construction_via_split/`
...
**S3 - Output stack: `ls` plus Enter and shell response**
...
- Do not pretend `src/main_ghostty.zig` is the user-visible app loop on macOS;
  the Swift main thread is the real app thread there.
...
- prefer short, code-anchored explanations over long prose
