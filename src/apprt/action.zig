...
pub const Action = union(Key) {
    ...
    /// Open a new window using an explicit surface configuration.
    new_window_with_surface_config: apprt.surface.SurfaceConfig,
    ...
    comptime {
        ...
        assert(@sizeOf(CValue) == @sizeOf(apprt.surface.SurfaceConfig));
    }
